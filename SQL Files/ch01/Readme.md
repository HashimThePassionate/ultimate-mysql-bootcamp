...
*The code creates and populates three separate databases*:
- sql_invoicing: Designed for invoice management and payment tracking.
- sql_store: Designed for managing products, customers, orders, and shipments.
- sql_hr: Designed for managing employee records and office locations.
...
- ### Tables:
- *payment_methods*: Stores available payment methods (e.g., credit card, cash).
- *clients*: Stores client information (e.g., name, address, phone).
- *invoices*: Stores invoice details (e.g., number, client, amount, dates).
- *payments*: Records payments made against invoices, including payment method.


## sql_store Database:

### Tables:
- *products*: Stores product information (e.g., name, quantity, price).
- *shippers*: Stores shipping company information.
- *customers*: Stores customer information (e.g., name, address, points).
- *order_statuses*: Stores possible order statuses (e.g., processed, shipped).
- *orders*: Stores order details (e.g., customer, date, status, shipper).
- *order_items*: Links orders to specific products with quantities and prices.
- *order_item_notes*: Stores notes associated with specific order items.

## sql_hr Database:

### Tables:
- *offices*: Stores office locations.
- *employees*: Stores employee information (e.g., name, title, salary, office, manager).

## Key Points:

- *Foreign keys*: Enforce relationships between tables for data integrity (e.g., orders references customers to ensure valid customer IDs).
- *Primary keys*: Uniquely identify each row in a table (e.g., employee_id in employees).
- *Normalization*: Tables are organized to minimize redundancy and improve efficiency.